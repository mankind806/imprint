...
| A human said yes before an irreversible outward action | **Reserved to a person** — the fourth state, and this row is why it is not "behaviour rule". A gate can block a destination: a branch protection rule, a deny entry, a missing credential. It cannot know whether anyone agreed, so what holds the rule is not an unbuilt mechanism but a handover, and the two fail differently — an unbuilt mechanism fails by never being built, a consent step fails by somebody deciding it was obvious enough to skip. `supersede-dont-delete` carries the full argument for the state. What *is* enforceable here and unbuilt is the surrounding shape: a token that a plan must produce before the action will run does not prove that anybody said yes, but it does stop the preparation and the firing from happening in one unattended move. |
