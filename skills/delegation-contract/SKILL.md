...
description: "Decides who leads and who advises before any agent is dispatched, writes the header telling the receiver which one it is, holds the invariant that readers run in parallel while exactly one writer holds the pen, and settles which model the dispatch goes to and how long that choice stays valid. Use when constructing a dispatch (receiver's role, write and outward rights, ambiguity policy, return format), when writing an orchestrator prompt, when two agents might touch the same files, when a dispatched agent reports unverified success, when you are unsure whether you lead or advise, when sending mechanical work to an expensive model or judgement to a cheap one, when a failed dispatch tempts an unchanged retry, when your model roster carries no check date, or when a delegated task could push, publish or deploy outside the repo irreversibly. Sending a diff out for review also fires blind-first-pass; both are correct — this one builds the envelope, that one decides what goes inside it. Not for choosing what a reviewer may see, whether a second opinion is worth having, or where a stronger voice earns its latency (use blind-first-pass), not for verifying a factual claim (use measure-before-asserting), not for what a closing session owes the next one (use session-handover, paired with this at a handover), and not for how a fact is stored, sourced, replaced or aged once it is written down (use one-canonical-place, provenance-on-entry, supersede-dont-delete and knowledge-ages)."
---
...
Sending a diff out for review typically fires this skill and `blind-first-pass` at the same
time, and that is correct rather than a conflict: this skill builds the envelope — the role,
the write and outward rights, the ambiguity policy, exactly one writer — and `blind-first-pass`
decides what goes inside that envelope, i.e. what the reviewer gets to see.
...
| A human said yes before an irreversible outward action | **Reserved to a person** — the fourth state, and this row is why it is not "behaviour rule". A gate can block a destination: a branch protection rule, a deny entry, a missing credential. It cannot know whether anyone agreed, so what holds the rule is not an unbuilt mechanism but a person's decision, and the two fail differently — an unbuilt mechanism fails by never being built, a consent step fails by somebody deciding it was obvious enough to skip. `supersede-dont-delete` carries the full argument for the state. What *is* enforceable here and unbuilt is the surrounding shape: a token that a plan must produce before the action will run does not prove that anybody said yes, but it does stop the preparation and the firing from happening in one unattended move. |

Read that table as four states rather than two: enforced, enforceable but not enforced,
reserved to a person, and plain behaviour rule. The network row is the second one in
